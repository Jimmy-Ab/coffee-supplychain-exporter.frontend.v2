...
import { ApproveContractDelivery, DeleteContractDelivery, GetAllContractDelivery, PayContractDelivery, RejectContractDelivery } from '@/api/contract-delivery';
...
export const ContractDeliveryCard:
    React.FC<Props> = ({
        ...
        const approving = useAppSelector(state => state.contractDelivery.approving);
        const paying = useAppSelector(state => state.contractDelivery.paying);
        const rejecting = useAppSelector(state => state.contractDelivery.rejecting);
        ...
        const HandleApprove = async () => {
            handleClose();
            setSelectedId(contractDelivery.Record.id);
            await dispatch(ApproveContractDelivery({
                id: contractDelivery.Record.id,
        ...
        const HandlePay = async () => {
            handleClose();
            setSelectedId(contractDelivery.Record.id);
            await dispatch(PayContractDelivery({
                id: contractDelivery.Record.id,
                contractId: contractDelivery.Record.contractId
            }));
            await dispatch(GetAllContractDelivery());
        ...
        const HandleReject = async () => {
            handleClose();
            setSelectedId(contractDelivery.Record.id);
            await dispatch(RejectContractDelivery({
                id: contractDelivery.Record.id,
                contractId: contractDelivery.Record.contractId
            }));
            await dispatch(GetAllContractDelivery());
                ...
                <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
                    <ListItem alignItems="flex-start"
                        ...
                        secondaryAction={
                            <>
                                {
                                    (approving || paying || deleting) && selectedId === contractDelivery.Record.id ?
                                        <CircularProgress />
                                        :
                                        <Button
                                            id="basic-button"
                                ...
                                >
                                    <MenuItem
                                        ...
                                        disabled={
                                            contractDelivery.Record.status !== 'REQUESTED'
                                        }
                                    >
                                        Approve
                                    </MenuItem>
                                    ...
                                    <MenuItem
                                        onClick={HandlePay}
                                        disabled={
                                            contractDelivery.Record.status !== 'APPROVED'
                                        }
                                    >
                                        Pay
                                    </MenuItem>
